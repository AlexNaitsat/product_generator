...
namespace UnityEngine.Perception.Randomization.Randomizers.SampleRandomizers
{
    ...
    /// </summary>
    [Serializable]
    [AddRandomizerMenu("My Randomizers/Fg Single Object Randomizer")]
    public class FgSingleObjectRandomizer : Randomizer
    {
        ...
        private string mainCameraRotationAxis = "x";
        ...
        public struct  PNGOutputFileSpec //generating unique names for PNG  output files
        {
            public PNGOutputFileSpec(int view_num_,string name_, string postfix_, int camera_num_, string camera_rot_axis_) 
            {
                view_num   = view_num_;
                name       = name_;
                postfix    = postfix_;
                camera_num = camera_num_;
                camera_rot_axis = camera_rot_axis_;
            }
            ...
            public string camera_rot_axis { get; }
            //generate relative path to rendered png in ShapeNetPlain format 
            public override string ToString() => $"{name}/frame_{view_num:D3}_{postfix}_{camera_rot_axis}{camera_num:D2}";
        }
        ...
        public static Bounds GetMaxBounds(GameObject g)
        {
            ...
            for (int i = 0; i < m.vertices.Length; ++i)
            {
                ...
                //     world_v1 = localToWorld.MultiplyPoint3x4(m.vertices[i]);//equivalent computation 
                for (int j = 0; j < 3; ++j)
                {
        ...
        /// </summary>
        // by the time OnIterationStart() is called, all public variables of components in the scene should be initialized.
        protected override void OnIterationStart() //this runs once per each object
        {   
            view_index =0;
            Debug.Log("FgSingleObjectRandomizer iter start prefab_index=" + prefab_index.ToString());
            ...
            mainCameraRotationAxis = mainCameraView.RotationAxis switch 
            { 
                ChangeCamera.Axis.x => "x",
                ChangeCamera.Axis.y => "y",
                ChangeCamera.Axis.z => "z",
                _ => throw new ArgumentOutOfRangeException(nameof(mainCameraView.RotationAxis), mainCameraView.RotationAxis, null)
            };


            //Debug.Log("mainCameraIndex = " + mainCameraIndex);
...

        {
            ...
            pngfileSpec[frame_index] = new PNGOutputFileSpec(view_index, current_object_name, current_postfix, mainCameraIndex, mainCameraRotationAxis);
